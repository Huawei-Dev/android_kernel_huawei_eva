...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8 hmac_tx_is_dhcp(mac_ether_header_stru  *pst_ether_hdr)
{
    ...
    oal_uint8               uc_ip_protocol;
    udp_hdr_stru           *pst_udp_hdr;

    puc_ip_hdr = (oal_uint8 *)(pst_ether_hdr + 1);
    uc_ip_protocol = *(puc_ip_hdr + IP_PROTOCOL_TYPE_OFFSET);

    if (MAC_UDP_PROTOCAL == uc_ip_protocol)
    {
        pst_udp_hdr = (udp_hdr_stru *)(puc_ip_hdr + IP_HDR_LEN);
        return mac_is_dhcp_port(pst_udp_hdr);
    }
    else
    {
        return OAL_FALSE;
...
OAL_STATIC OAL_INLINE oal_void  hmac_tx_classify_lan_to_wlan(oal_netbuf_stru *pst_buf, oal_uint8 *puc_tid)
{
    ...
    switch (pst_ether_header->us_ether_type)
    {
        ...
        case OAL_HOST2NET_SHORT(ETHER_TYPE_IP):
            ...
            {
                udp_hdr_stru *pst_udp_hdr = (udp_hdr_stru *)(pst_ip + 1);
                /* 如果是DHCP帧，则进入VO队列发送 */
